...
void solve(char l[],int a,int b){
	...
	bool flag=true;
	i=0;
	while(lparen.size()&&i<lparen.size()){
		//cout<<"In paren"<<endl;
		if(lparen[i]>=a&&lparen[i]<=b){
			start=lparen[i];
			end=rparen[i];
			lparen.erase(lparen.begin()+i);
			rparen.erase(rparen.begin()+i);
			solve(l,start,end);
			l[start]=-5;
			l[end]=-5;
			p[start]=p[start+1];
...
int main(){
	...
	while(t--){
		...
		if(n==1){
			...
			//cout<<endl;
			qinv=modInverse(power(4,hsh.size()),mod);
			cout<<(res[0][0]*qinv)%mod<<" ";
			cout<<(res[0][1]*qinv)%mod<<" ";
			cout<<(res[0][2]*qinv)%mod<<" ";
			cout<<(res[0][3]*qinv)%mod<<" ";
		}
		cout<<endl;
